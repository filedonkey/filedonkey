...
static void Start(VirtDisk *self, Connection *conn)
{
    ...
    std::string volname = QString("volname=%1").arg(conn->machineName).toStdString();

    char *argv[] = {"FileDonkey", "-o", volname.data(), "-o", "subtype=filedonkey", "-o", "auto_cache"};
    int argc = sizeof(argv) / sizeof(argv[0]);
    struct fuse_args args = FUSE_ARGS_INIT(argc, argv);

    qDebug() << "[Start] fuse_pkgversion" << fuse_pkgversion();
    qDebug() << "[Start] fuse_version" << fuse_version();
